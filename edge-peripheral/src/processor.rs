...
impl Processor for DebugProcessor {
    async fn awaiting_time_sync(&self, 
        ...
        let station_state = ble::StationState::init_with_mac(mac);
        let station_state = ble::run(controller, &station_state, Some(rtc))
            .await
            .map_err(|e| anyhow::anyhow!("BLE time sync failed: {e:?}"))?;

        if station_state.current_time.timestamp != 0 {
            info!("Time synced to unix {}", station_state.current_time.timestamp);
        } else {
    ...
    async fn flushing(
        ...
        info!(
            "Syncing {} measurement bucket(s) over BLE",
            state.buckets.len()
        );

        let station_state = ble::StationState::from_measurements(mac, state)
            .map_err(|_| anyhow::anyhow!("Failed to encode measurements for BLE"))?;

        ble::run(controller, &station_state, None)
            .await
            .map_err(|e| anyhow::anyhow!("BLE sync failed: {e:?}"))?;

        Ok(state::DeviceState::Buffering(timeseries::Series::new(
            edge_protocol::Measurement::MAX_DEVIATION,
        )))
    }
}
