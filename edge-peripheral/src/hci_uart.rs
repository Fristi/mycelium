...
pub(crate) struct UartReader(UartRx<'static, Async>);
pub(crate) struct UartWriter(UartTx<'static, Async>);
...
pub type UartTransport =
    SerialTransport<NoopRawMutex, UartReader, UartWriter>;

pub fn init_hci_uart(
    uart: esp_hal::peripherals::UART0<'static>,
    tx: esp_hal::peripherals::GPIO1<'static>,
    rx: esp_hal::peripherals::GPIO3<'static>,
) -> anyhow::Result<ExternalController<UartTransport, 20>> {
    ...
    Ok(ExternalController::new(SerialTransport::new(
        UartReader(rx),
        UartWriter(tx),
    )))
}
